...
do
    ...
    res=`timeout --signal=KILL 20m ./$process $inputGraph output_${process}_${numThread}Thread -GMInputFormat="edge" -GMMeasureTime=1 -GMNumThreads=$numThread -num=${setNum} >> OPENMP_${process}_${graphName}_Results`
